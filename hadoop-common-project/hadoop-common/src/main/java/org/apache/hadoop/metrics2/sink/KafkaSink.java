...
public class KafkaSink implements MetricsSink, Closeable {
    ...
    public void putMetrics(MetricsRecord record) {
        ...
        for (AbstractMetric metric : record.metrics()) {
            ...
            jsonLines.append("\", \"name\": \"" + metricsPathPrefix.toString() + "." + metric.name().toString().replaceAll("[\\p{Cc}]", ""));
            jsonLines.append("\", \"message\": \"" + metric.value().toString());
            jsonLines.append("\", \"level\": \"null\"}");
            // jsonLines.append("\n");
